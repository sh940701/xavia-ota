...
We provide a script `build-and-publish-app-release.sh` in the `scripts` folder to build and publish your app updates. Copy it to your RN app root folder and run it from there.
...
3. Upload it to your Xavia OTA server (`POST /api/upload`)

The script shows commit hash and message for confirmation before upload.
...
- `POST /api/login` validates `ADMIN_PASSWORD` and sets `xavia_admin_session` (`HttpOnly`) cookie.
- Session cookie signature uses `ADMIN_SESSION_SECRET` (fallback to `ADMIN_PASSWORD` for backward compatibility).
- `GET /api/releases`, `POST /api/rollback`, and `GET /api/tracking/*` require a valid admin session cookie.
- `POST /api/upload` remains public to automation flows and is protected by `UPLOAD_KEY`.
